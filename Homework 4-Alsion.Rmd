---
title: "Homework 4"
author: " ESM 204 HW 4: Calculating the SCC and policy choice under uncertainty"
date: "5/28/2021"
...
# Question 2 Plot 4
plot_difference_per_ton <-
  ggplot(data = warming_diff_norm, aes(x = year, y = damage_diff_norm)) +
 geom_point() +
 labs(x = "Year", y = " Difference in damage ($ per ton)") +
theme_minimal()
...
scc_rams_plot <- ggplot() +
  ...
  geom_point( data = scc_rams_df, aes( x= rate, y= scc_sum, color="tomato2")) +
...
max_price_x
```
If society is risk neutral, policy B would make economic sense as long as it is less than or equal to $1.979412e+15 over the 80 year timeframewfs . If the cost of  implementing policy B is above 1.979412e+15, then policy A would make more economic sense.
If the society is risk adverse, the maximum price of x could increase because the utility of guaranteed temp. reduction in B is more than the utility of policy a which involves risk. 
If society is instead risk adverse, society is willing to pay more for poLicy b (up to a certain point) as it eliminates any risk that is present in the business as usual  Policy A. The certain point to which a risk adverse society would switch to policy a would be when the price of x would  the utility of policy of A is greater than the utility of policy B
if a and b +x are equal the osciety will ooick b as the utility